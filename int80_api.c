...
void int80_free_message(int80_message *msg) {
	printf("params copy: %d, params ref: %d, params: %X\n",
		msg->params_copied, msg->params_ref, msg->params);
...
void int80_set_message_command_copy(int80_message *msg, const char *command) {
	...
	memcpy(msg->command, command, strlen(command));
    (msg->command)[strlen(command)] = '\0';
